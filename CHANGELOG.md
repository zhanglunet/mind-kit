...
### 公开包收敛为 Vault-only（如实说明）

- **v1.7 曾把飞书同步连接器随公开仓发布;本版起公开包收回该能力,恢复
  「公开工具集 + 私有发行包」的边界**:公开安装器仍提供一键初始化本地
  Vault(macOS / Linux / Windows 原生 PowerShell),但不含飞书授权与内容
  同步——安装器页面与 `--self-check` 会明确提示当前包的能力范围。
- 飞书云文档 / 知识库备份改经**完整发行包**(邀请制内测,免费)交付,
  申请方式见文档站「服务」页。v1.7 已发布的连接器代码仍存在于本仓历史中,
  仅从当前版本移除;此前按 v1.7 部署的用户可继续使用,或申请完整发行包
  获得后续更新。
- 站点与指南(服务页 / Workbuddy 向导 / 安装指南 / README)同步改写,
  与上述边界一致;不再宣称"同步能力已进公开仓"。

### 依赖与质量

- lark-cli 安装锁定版本(`@larksuite/cli@1.0.87`),不再裸包名安装;
  升级须实测后同步安装器与手册两处(发行门禁锁一致)。
- 安装器的 lark-cli 命令构造收敛为单一公开模块,消除公开/私有两份实现的
  语义漂移(空白 profile 归一、Windows 可执行解析优先级统一)。
