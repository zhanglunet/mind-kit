...
### WorkBuddy 技能包 v1.0.1：说明书跟上 Vault-only 边界

v1.8 把公开包收敛为 Vault-only，但站点下载的技能包里，给 AI 助手看的说明书
仍在教「用公开仓做飞书授权和同步」。照它走会在配置飞书应用那步撞上
「当前公开发行包不含同步连接器」——**看起来像产品坏了，其实是设计如此**。

v1.0.1 改为先分清两种发行包再分岔：公开包认页面标题即跳到编译；
授权与同步整节标注「仅完整发行包」，并写明公开包上的那句报错是边界而非故障。
技能名去掉「与飞书备份」。安全边界六条一字未动。

### 技能包从此有源可查

此前这个 zip **只有二进制、没有源**：内容进不了 code review，又因为是压缩容器
而被禁忌词门禁豁免——全仓唯一「有对外内容却对所有检查不可见」的东西。
v1.8 改了六处文案唯独漏掉它，根因就在这里。

现在改为：`skills/workbuddy-second-brain/` 纯文本源（门禁扫得到）+
`scripts/build-skill-package.py` 确定性构建（同源必同字节）+
`tests/test_skill_package.py` 锁「产物与源一致、无过期版本并存、说明书含边界表述」。
想改技能包，改源、跑构建脚本，别再手工解包重打包。

### 移除退役的 expert 包

`workbuddy-second-brain-expert-v1.0.0.zip` 是包形态统一之前的遗留，此后全仓零引用，
却仍随公开版发布且内容过期。已删除，二进制豁免规则同步收窄为「仅技能包」。
