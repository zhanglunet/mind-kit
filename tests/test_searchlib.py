...
# ---------- snippet:给人看的,不是给机器看的 ----------

def test_snippet_skips_frontmatter():
    """摘要必须跳过 frontmatter。

    实测代价(2026-07-26 飞书机器人上线首日):snippet 取 content[:120],而库里页面
    普遍以 10+ 行 frontmatter 开头 → 卡片里全是「类型:/批次:/来源:/ingested:/tags」,
    正文一个字没有。更糟的是 frontmatter 里的 feishu_token 被原样推进了聊天窗口。
    """
    doc = {"path": "a.md", "title": "标题",
           "content": ("---\n类别: ⑤ 框架与心智模型\n主体: 某人\n"
                       "feishu_token: SecretLookingToken123\ntags: [x]\n---\n\n"
                       "# 三大协议对比\n\n真正的正文从这里开始,讲的是协议差异。\n")}
    idx = S.build_index([doc])
    snip = idx["entries"][0]["snippet"]
    assert "真正的正文" in snip
    assert "feishu_token" not in snip, "frontmatter 不得进摘要(曾泄 token 到聊天窗口)"
    assert "类别" not in snip and "tags" not in snip


def test_snippet_skips_leading_heading():
    doc = {"path": "b.md", "title": "标题", "content": "# 大标题\n\n正文内容在此。\n"}
    snip = S.build_index([doc])["entries"][0]["snippet"]
    assert snip.startswith("正文内容"), f"标题行不该占摘要:{snip!r}"


def test_snippet_falls_back_when_only_frontmatter():
    """只有 frontmatter 没正文时不能给空串(卡片会出现空行)。"""
    doc = {"path": "c.md", "title": "标题", "content": "---\n类别: x\n---\n"}
    assert S.build_index([doc])["entries"][0]["snippet"] != ""


def test_snippet_is_single_line():
    """摘要进卡片,换行会把版式撑乱。"""
    doc = {"path": "d.md", "title": "T", "content": "第一行\n第二行\n第三行\n"}
    assert "\n" not in S.build_index([doc])["entries"][0]["snippet"]


def test_frontmatter_still_searchable():
    """只清摘要,不动召回:frontmatter 里的词仍要能搜到(来源/tags 是有用的检索面)。"""
    doc = {"path": "e.md", "title": "无关标题",
           "content": "---\n来源: 飞书云文档\n---\n\n正文\n"}
    idx = S.build_index([doc])
    assert S.search(idx, "飞书云文档"), "frontmatter 仍应参与召回"
