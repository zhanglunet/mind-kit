...
> **Workbuddy 用户**：Windows 10/11 不需要 WSL。请直接看
> [Workbuddy 安装向导](workbuddy.md)——公开仓安装器为 **Vault-only**（初始化本地
> Vault，不含飞书同步）；飞书授权与同步验收流程仅适用于完整发行包（邀请制，
> 见[服务页](services.md)）。macOS/Linux 也可以使用同一页的提示词流程。
